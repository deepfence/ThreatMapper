...
func init() {
	os.Mkdir("/tmp/ubuntu-cve-tracker", 0755)
	vulnsrc.RegisterUpdater("ubuntu", &updater{repositoryLocalPath: "/tmp/ubuntu-cve-tracker"})
}
...
func (u *updater) Update(datastore database.Datastore) (resp vulnsrc.UpdateResponse, err error) {
	...
	if err != nil {
		errStr = fmt.Sprintf("Error while pulling repository. %s", err.Error())
		log.Info(errStr)
		return
	}
	...
	if err != nil {
		errStr = fmt.Sprintf("Error getting commit id from db. %s", err.Error())
		log.Info(errStr)
		return resp, err
	}
	...
	if err != nil {
		log.Info(fmt.Sprintf("Error collecting vulnerabilities.  %s", err.Error()))
		return resp, err
	}
	...
	for cvePath := range modifiedCVE {
		...
		if err != nil {
			...
			// commit.
			log.Info(fmt.Sprintf("error opening %s. %s", u.repositoryLocalPath+"/"+cvePath, err.Error()))
			continue
		}
		// Parse the vulnerability.
		v, unknownReleases, err := parseUbuntuCVE(file)
		if err != nil {
			log.Info(fmt.Sprintf("Parse cve error.%s", err.Error()))
			return resp, err
		}
		...
		for k := range unknownReleases {
			...
			/* *** DO NOT RESET COMMIT ID WHEN UNKNOWN VERSION IS FOUND ***
			commit = dbCommit
			*/
		}
...
func (u *updater) pullRepository() (commit string, err error) {
	...
	var errStr string
	filePtr, openErr := os.Open(u.repositoryLocalPath)
	if (openErr != nil) && (openErr != io.EOF) {
		return "", commonerr.ErrCouldNotDownload
	}
	defer filePtr.Close()
	_, openErr = filePtr.Readdirnames(1)
	if openErr == io.EOF {
	...
	if err != nil {
		errStr = fmt.Sprintf("Error while git. Reason = %s", err.Error())
		log.Info(errStr)
		return "", vulnsrc.ErrGitFailure
	}
