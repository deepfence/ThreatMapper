...
func init() {
	os.Mkdir("/tmp/alpine-secdb-data", 0755)
	vulnsrc.RegisterUpdater("alpine", &updater{repositoryLocalPath: "/tmp/alpine-secdb-data"})
}
...
func (u *updater) processFile(filename string) {
	...
	if nameParts[1] == "json" {
		log.WithField("package", "Alpine").Error("Filename is not json extension" + filename)
		return
	}
	response, err := http.Get(baseURL + u.currentDir + filename)
	if err != nil {
		log.WithField("package", "Alpine").Error("Failed to get vuln file")
		return
	}
	if response == nil {
		log.WithField("package", "Alpine").Error("Failed to get vuln file. Response is Null")
		return
	}
...
func (u *updater) processVersionDir(versionDir string) {
	...
	if response == nil {
		log.WithError(err).WithField("package", "Alpine").Error("Failed to get version. Response is Null")
		return
	}
	defer response.Body.Close()
...
func (u *updater) getVulnFiles(repoPath, tempDirPrefix string) (commit string, err error) {
	...
	if err != nil {
		log.WithError(err).WithField("package", "Alpine").Fatal("Error loading HTTP response body. " + err.Error())
		return
	}
	if response == nil {
		log.WithError(err).WithField("package", "Alpine").Fatal("Error loading HTTP response body. Response is nil")
		return
	}
	defer response.Body.Close()
...
func (u *updater) Update(db database.Datastore) (resp vulnsrc.UpdateResponse, err error) {
	...
	if err != nil {
		log.WithError(err).WithField("package", "Alpine").Fatal("Error reading commit id from db " + err.Error())
		return
	}
	...
	if err != nil {
		log.WithError(err).WithField("package", "Alpine").Fatal("Error doing ls " + err.Error())
		return
	}
	...
	for _, namespace := range namespaces {
		...
		if err != nil {
			log.WithError(err).WithField("package", "Alpine").Fatal("Error parsing vulnerabilities " + err.Error())
			return
		}
		if note != "" {
