...
then
    ...
    KAFKA_ADVERTISED_LISTENERS="PLAINTEXT://deepfence-kafka-broker-$POD_NUMBER.$SERVICE.$NAMESPACE.svc:9092"

    KAFKA_CONTROLLER_QUORUM_VOTERS=""
    for i in $( seq 0 $REPLICAS); do
        if [[ $i != $REPLICAS ]]; then
            KAFKA_CONTROLLER_QUORUM_VOTERS="$KAFKA_CONTROLLER_QUORUM_VOTERS$((i+1))@deepfence-kafka-broker-$i.$SERVICE.$NAMESPACE.svc:9093,"
        else
