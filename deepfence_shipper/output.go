...
func (p *Publisher) validateTokens() error {
	...
	if !deepfenceUtils.IsJWTExpired(p.AccessToken) {
		return nil
	} else {
...
func NewPublisher(cfg Console) *Publisher {
...
func (p *Publisher) Publish(ctx context.Context, entry FileEntry, tail *tail.Tail) {
	...
	log.Printf("start publish for %s -> %s", entry.LocalPath, entry.RemotePath)

	for {
		select {
		case <-ctx.Done():
			log.Printf("stopping publish for %s -> %s", entry.LocalPath, entry.RemotePath)
			return
...
func (p *Publisher) pushdata(target string, data []map[string]interface{}) error {
	...
	log.Printf("pushing #data %d to url %s", len(data), url)
