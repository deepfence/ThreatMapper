...
func (nc *neo4jIngester) applyDBConstraints() error {
	...
	session.Run("CREATE CONSTRAINT ON (n:Cve) ASSERT n.node_id IS UNIQUE", map[string]interface{}{})
	...
	session.Run("CREATE CONSTRAINT ON (n:Registry) ASSERT n.node_id IS UNIQUE", map[string]interface{}{})
	...
	runComplianceInitialization(session)

	return nil
}

func runComplianceInitialization(session neo4j.Session) {
	session.Run("MERGE (n:AWSComplianceNode{node_id:'aws-compliance-node', cloud_provider:'aws'})", map[string]interface{}{})
	session.Run("MERGE (n:GCPComplianceNode{node_id:'gcp-compliance-node', cloud_provider:'gcp'})", map[string]interface{}{})
	session.Run("MERGE (n:AzureComplianceNode{node_id:'azure-compliance-node', cloud_provider:'azure'})", map[string]interface{}{})
	session.Run("MERGE (n:AWSComplianceBenchmark{node_id: $node_id, version: $version, title: $title, description: $description})",
	...
	session.Run("MERGE (n:AWSComplianceBenchmark{node_id: $node_id, title: $title})",
		map[string]interface{}{"node_id": "aws-compliance-cis-v150-1", "title": "1 Identity and Access Management"})
	session.Run("MERGE (n:AWSComplianceControl{node_id: $node_id, title: $title, description: $description, cis_type: $cis_type, service: $service})",
