...
func (qs *AsynqQueueState) Run() {
	...
	ns := string(qs.namespace)

	configs, err := directory.GetDatabaseConfig(ctx)
	if err != nil {
		log.Error().Str("namespace", ns).Err(err).Msg("failed to get db configs")
		return
	}
	hostPort := strings.Split(configs.Neo4j.Endpoint, ":")
	if len(hostPort) != 3 {
		log.Error().Str("namespace", ns).Msgf("Invalid neo4j endpoint %v", configs.Neo4j.Endpoint)
		return
	}
	err = utils.WaitServiceTCPConn(hostPort[1][2:], hostPort[2], qs.timeout)
	if err != nil {
		log.Warn().Str("namespace", ns).Msgf("pause queues neo4j not reachable %s", err)
		AsynqPauseQueues(ctx)
	} else {
		log.Warn().Str("namespace", ns).Msgf("unpause queues neo4j reachable")
...
func AsynqPauseQueues(ctx context.Context) error {
	...
	for _, q := range queues {
		if info, err := worker.Inspector().GetQueueInfo(q); err != nil {
			log.Error().Str("namespace", namespace).Err(err).Msgf("failed to get queue info %s", q)
		} else {
			if !info.Paused {
				log.Warn().Str("namespace", namespace).Msgf("pause worker queue %s", q)
				if err := worker.Inspector().PauseQueue(q); err != nil {
					log.Error().Str("namespace", namespace).Err(err).Msgf("failed to pause worker queue %s", q)
				}
			}
		}
	}
...
func AsynqUnpauseQueues(ctx context.Context) error {
	...
	for _, q := range queues {
		if info, err := worker.Inspector().GetQueueInfo(q); err != nil {
			...
			if info.Paused {
				log.Warn().Str("namespace", namespace).Msgf("unpause worker queue %s", q)
				if err := worker.Inspector().UnpauseQueue(q); err != nil {
					log.Error().Str("namespace", namespace).Err(err).Msgf("failed to unpause worker queue %s", q)
				}
			}
		}
	}
