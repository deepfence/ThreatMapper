...
func IsApplication(n report.Node) bool {
	...
	roleLabel, _ := n.Latest.Lookup(report.DockerLabelPrefix + "deepfence.role")
	if roleLabel == "system" {
		return false
	}
	roleLabel, _ = n.Latest.Lookup(report.DockerImageLabelPrefix + "deepfence.role")
	if roleLabel == "system" {
		return false
	}
	containerNameLabel, _ := n.Latest.Lookup(report.DockerLabelPrefix + "io.kubernetes.container.name")
	if containerNameLabel == "POD" {
		return false
	}
...
var (
	topologyIdOrder = map[string]int{report.CloudProvider: 1, report.CloudRegion: 2, report.KubernetesCluster: 3, report.Host: 3, report.Service: 4, report.Pod: 5, report.Container: 6, report.Process: 7}
	parentOrder     = map[string]map[string]struct{}{
		report.Process:           {report.Container: struct{}{}, report.Pod: struct{}{}, report.Host: struct{}{}, report.KubernetesCluster: struct{}{}, report.CloudRegion: struct{}{}, report.CloudProvider: struct{}{}},
		report.Container:         {report.Pod: struct{}{}, report.Host: struct{}{}, report.KubernetesCluster: struct{}{}, report.CloudRegion: struct{}{}, report.CloudProvider: struct{}{}},
		report.Pod:               {report.Host: struct{}{}, report.KubernetesCluster: struct{}{}, report.CloudRegion: struct{}{}, report.CloudProvider: struct{}{}},
		report.Service:           {report.KubernetesCluster: struct{}{}, report.CloudProvider: struct{}{}},
		report.Host:              {report.KubernetesCluster: struct{}{}, report.CloudRegion: struct{}{}, report.CloudProvider: struct{}{}},
		report.KubernetesCluster: {report.CloudProvider: struct{}{}},
		report.CloudRegion:       {report.CloudProvider: struct{}{}},
	}
)

func IsParent(parentTopologyName, parentID string) FilterFunc {
	return func(n report.Node) bool {
		parentIDs, found := n.Parents.Lookup(parentTopologyName)
		if !found {
			return false
		}
...
func IsImmediateParent(parentTopologyName string) FilterFunc {
	return func(n report.Node) bool {
		possibleParentTypes, ok := parentOrder[n.Topology]
		if !ok {
			return false
		}
		immediateParent := report.CloudProvider
		for _, parentTopologyType := range n.Parents.Keys() {
			if _, ok := possibleParentTypes[parentTopologyType]; ok {
				if topologyIdOrder[parentTopologyType] >= topologyIdOrder[immediateParent] {
					immediateParent = parentTopologyType
				}
			}
		}
		if immediateParent == parentTopologyName {
			return true
		}
		return false
	}
...
// IsMetadata checks if the node has specified metadata
func IsMetadata(key, val string) FilterFunc {
	return func(n report.Node) bool {
		latestVal, found := n.Latest.Lookup(key)
		if !found {
			return false
		}
		if latestVal != val {
