...
var renderers = map[string]func(BasicNodeSummary, report.Node) BasicNodeSummary{
	...
	report.CloudProvider:         cloudProviderNodeSummary,
	report.CloudRegion:           cloudRegionNodeSummary,
	report.KubernetesCluster:     kubernetesClusterNodeSummary,
...
var primaryAPITopology = map[string]string{
	...
	report.CloudProvider:         "cloud-providers",
	report.CloudRegion:           "cloud-regions",
	report.KubernetesCluster:     "kubernetes-clusters",
	report.Pod:                   "pods",
...
func MakeNodeSummary(rc RenderContext, n report.Node, ignoreMetadata bool, ignoreMetrics bool) (NodeSummary, bool) {
...
func pseudoNodeSummary(base BasicNodeSummary, n report.Node) BasicNodeSummary {
	...
	var prefix, shape string
	var found bool
	if n.ID != render.IncomingInternetID && n.ID != render.OutgoingInternetID {
		prefix, shape, found = matchesKnownNodes(n)
	}
...
func matchesKnownNodes(n report.Node) (string, string, bool) {
	for _, serviceRegexpMatcher := range render.KnownServiceRegexpMatchers {
		if strings.HasPrefix(n.ID, serviceRegexpMatcher.Prefix) {
			return serviceRegexpMatcher.Prefix, serviceRegexpMatcher.Name, true
		}
	}
	return "", "", false
}
...
func containerNodeSummary(base BasicNodeSummary, n report.Node) BasicNodeSummary {
	var (
		...
		imageTag, _      = n.Latest.Lookup(docker.ImageTag)
		imageNameWithTag = fmt.Sprintf("%s:%s", imageName, imageTag)
	)
	...
	base.Image = imageNameWithTag
	return base
}

func kubernetesClusterNodeSummary(base BasicNodeSummary, n report.Node) BasicNodeSummary {
	var kubernetesClusterNameName, _ = n.Latest.Lookup(report.KubernetesClusterName)
	base.Label = kubernetesClusterNameName
	base.LabelMinor = ""
	base.Rank = kubernetesClusterNameName
	base.Shape = report.KubernetesCluster
	return base
}

func cloudProviderNodeSummary(base BasicNodeSummary, n report.Node) BasicNodeSummary {
	var cloudProviderName, _ = n.Latest.Lookup("name")
	var cloudProviderLabel, _ = n.Latest.Lookup("label")
	base.Label = cloudProviderLabel
	base.LabelMinor = ""
	base.Rank = cloudProviderName
	base.Shape = cloudProviderName
	return base
}

func cloudRegionNodeSummary(base BasicNodeSummary, n report.Node) BasicNodeSummary {
	var cloudRegionName, _ = n.Latest.Lookup("name")
	var cloudProviderName, _ = n.Latest.Lookup(report.CloudProvider)
	base.Label = cloudRegionName
	base.LabelMinor = cloudProviderName
	base.Rank = cloudRegionName
	base.Shape = cloudProviderName + "-" + report.CloudRegion
	return base
}
...
func containerImageNodeSummary(base BasicNodeSummary, n report.Node) BasicNodeSummary {
	...
	case imageName != "" && imageName != ImageNameNone && imageTag != "" && imageTag != ImageNameNone:
		base.Label = imageNameWithTag
	default:
...
func hostNodeSummary(base BasicNodeSummary, n report.Node) BasicNodeSummary {
	var (
		...
		cloudProvider, _ = n.Latest.Lookup(report.CloudProvider)
	)
	if len(parts) == 2 {
		base.Label, base.LabelMinor, base.Rank = hostname, parts[1], parts[1]
	} else {
		base.Label = hostname
	}
	if cloudProvider == report.CloudProviderServerless {
		base.Shape = report.CloudProviderServerless
	}
	return base
}
...
func Summaries(ctx context.Context, rc RenderContext, rns report.Nodes, adjacency bool) NodeSummaries {
	span, ctx := opentracing.StartSpanFromContext(ctx, "detailed.Summaries")
	defer span.Finish()
	var ignoreMetadata, ignoreMetrics bool
	ignoreMetadata = false
	ignoreMetrics = false
	//ignoreMetadataInterface := ctx.Value(render.IgnoreMetadata)
	//if ignoreMetadataInterface == nil {
	//	ignoreMetadata = false
	//} else {
	//	ignoreMetadata = ignoreMetadataInterface.(bool)
	//}
	//ignoreMetricsInterface := ctx.Value(render.IgnoreMetrics)
	//if ignoreMetricsInterface == nil {
	//	ignoreMetrics = false
	//} else {
	//	ignoreMetrics = ignoreMetricsInterface.(bool)
	//}
