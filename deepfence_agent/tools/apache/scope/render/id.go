...
func externalNodeID(rpt report.Report, n report.Node, addr string, local report.Networks) (string, bool) {
	...
	hostname, serviceType := rpt.DNS.FirstMatch(n.ID, isKnownService)
	for _, serviceRegexpMatcher := range KnownServiceRegexpMatchers {
		if serviceType == serviceRegexpMatcher.Prefix {
			return serviceRegexpMatcher.Prefix + hostname, true
		}
	}
	if serviceType == ServiceNodeIDPrefix {
