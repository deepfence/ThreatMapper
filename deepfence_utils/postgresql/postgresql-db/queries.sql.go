...
const getUnreadDeepfenceCommunication = `-- name: GetUnreadDeepfenceCommunication :many
SELECT id, title, content, link, link_title, button_content, read, created_at, updated_at
FROM deepfence_communication
WHERE read = false
ORDER BY created_at
`

func (q *Queries) GetUnreadDeepfenceCommunication(ctx context.Context) ([]DeepfenceCommunication, error) {
	rows, err := q.db.QueryContext(ctx, getUnreadDeepfenceCommunication)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []DeepfenceCommunication
	for rows.Next() {
		var i DeepfenceCommunication
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.Content,
...
const markDeepfenceCommunicationRead = `-- name: MarkDeepfenceCommunicationRead :exec
UPDATE deepfence_communication
SET read = true
WHERE id = $1
`

func (q *Queries) MarkDeepfenceCommunicationRead(ctx context.Context, id int64) error {
	_, err := q.db.ExecContext(ctx, markDeepfenceCommunicationRead, id)
	return err
}
...
const upsertDeepfenceCommunication = `-- name: UpsertDeepfenceCommunication :one
...
ON CONFLICT (id) DO UPDATE
    SET title          = $2,
        ...
        link_title     = $5,
        button_content = $6
RETURNING id, title, content, link, link_title, button_content, read, created_at, updated_at
`

type UpsertDeepfenceCommunicationParams struct {
	ID            int64  `json:"id"`
...
func (q *Queries) UpsertDeepfenceCommunication(ctx context.Context, arg UpsertDeepfenceCommunicationParams) (DeepfenceCommunication, error) {
	row := q.db.QueryRowContext(ctx, upsertDeepfenceCommunication,
		arg.ID,
		arg.Title,
		arg.Content,
		arg.Link,
