...
import { ModelScanInfo, ModelSecretScanResult, ResponseError } from '@/api/generated';
...
async function getScanSummary(scanIds: string[]): Promise<ScanData[]> {
  ...
  const resultData = responses.map((response) => {
    ...
    return {
      accountName: getAccountName(response.value),
      accountType: response.value.node_type,
      data: [
        {
          total: Object.keys(response.value.severity_counts ?? {}).reduce(
            (acc, severity) => {
              acc = acc + (response.value.severity_counts?.[severity] ?? 0);
              return acc;
            },
            0,
          ),
          counts: Object.keys(response.value.severity_counts ?? {}).map((severity) => {
            return {
              name: severity,
              value: response.value.severity_counts?.[severity] ?? 0,
            };
          }),
        },
      ],
...
async function getScanStatus(bulkScanId: string): Promise<Array<ModelScanInfo>> {
  const statusScanApi = apiWrapper({
    fn: statusScanApiFunctionMap['secret'],
  });
