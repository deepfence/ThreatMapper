...
type ScanResult = {
  ...
  nodeName: string;
  description: string;
  compliances: ModelComplianceScanResult['compliances'];
  pagination: {
    currentPage: number;
    totalRows: number;
  };
...
async function getScans(scanType: string, scanId: string): Promise<ScanResult> {
  const filterParams = {
    fields_filter: {
      contains_filter: {
        filter_in: {},
  ...
  return {
    ...
    tableData: compliances as ModelCompliance[],
    pagination: {
      currentPage: page,
      // totalRows: page * PAGE_SIZE + resultCounts.count,
      totalRows: 15,
...
const loader = async ({
  ...
  return typedDefer({
    data: getScans('compliance', scanId),
  });
};
...
const CVETable = () => {
  ...
  const columns = useMemo(() => {
    const columns = [
      ...
      columnHelper.accessor('compliance_node_type', {
        enableSorting: false,
        enableResizing: false,
        cell: (info) => {
          ...
          <Badge
            ...
            className={cx({
              ...
              'bg-[#d6e184]/20 dark:bg-[#d6e184]/10 text-yellow-300 dark:text-[#d6e184]':
                info.getValue().toLowerCase() === 'warn',
