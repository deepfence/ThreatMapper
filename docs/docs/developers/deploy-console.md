...
2. Install your Management Console
    ...
    ```bash
    helm install -f deepfence_console_values.yaml deepfence-console deepfence/deepfence-console --version 2.0.0
    ```
...
4. Optional: enable external access with the `deepfence-router` package:
    ...
    ```bash
    helm install -f deepfence_router_values.yaml deepfence-router deepfence/deepfence-router --version 2.0.0
    ```
