...
:::info
Image tag `khulnasoft/deepfence_agent_ce:THREATMAPPER_VERSION-multiarch` is supported in amd64 and arm64/v8 architectures.
:::
...
docker run -dit \
    ...
    khulnasoft/deepfence_agent_ce:THREATMAPPER_VERSION
```
...
sudo podman run -dit \
    ...
    -e MGMT_CONSOLE_PORT="443" \
    -e DEEPFENCE_KEY="---DEEPFENCE-API-KEY---" \
    docker.io/khulnasoft/deepfence_agent_ce:THREATMAPPER_VERSION
```
