...
dependency_check_file=$(pwd)/clair/dependency-check-6.5.0-release.zip
if [ ! -f "$dependency_check_file" ]; then
    wget https://github.com/jeremylong/DependencyCheck/releases/download/v6.5.0/dependency-check-6.5.0-release.zip -P "$(pwd)/clair/"
    if [ ! $? -eq 0 ]; then
        exit 1
    fi
fi
...
cp clair/dependency-check-6.5.0-release.zip $DEEPAUDIT_DIR
