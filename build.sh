...
if [ ! -f certs/ssl/filebeat.crt ]; then
    ...
    mkdir -p certs/ssl
    sudo openssl genrsa -out certs/ssl/filebeat.key 1024
    sudo openssl req -new -key certs/ssl/filebeat.key -out certs/ssl/filebeat.csr
    sudo openssl x509 -req -days 365 -in certs/ssl/filebeat.csr -signkey certs/ssl/filebeat.key -out certs/ssl/filebeat.crt
    sudo chmod a+r certs/ssl/filebeat*
else
    echo "SSL certificate found"
fi

dependency_check_file=$(pwd)/clair/dependency-check-6.3.2-release.zip
if [ ! -f "$dependency_check_file" ]; then
    wget https://github.com/jeremylong/DependencyCheck/releases/download/v6.3.2/dependency-check-6.3.2-release.zip -P "$(pwd)/clair/"
    if [ ! $? -eq 0 ]; then
        exit 1
    fi
fi
...
cp clair/dependency-check-6.3.2-release.zip $DEEPAUDIT_DIR
...
docker build -f $DEEPFENCE_BACKEND_DIR/dockerify/api/Dockerfile.steampipe -t ${IMAGE_REPOSITORY:-deepfenceio/}steampipe:latest $DEEPFENCE_BACKEND_DIR
