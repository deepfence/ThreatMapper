...
from utils.helper import websocketio_channel_name_format, get_image_cve_status, get_image_secret_status, get_image_malware_status
...
def get_scan_status_for_registry_images(registry_image_list, image_cve_status=None):
    ...
    image_malware_status = get_image_malware_status()
    ...
    for reg_image in registry_image_list:
        ...
        malware_status = image_malware_status.get(reg_image["image_name_with_tag"], {})
        if cve_status:
        ...
        if malware_status:
            reg_image["malware_scan_status"] = status_map.get(
                malware_status["scan_status"], cve_never_scanned)
            reg_image["malware_scan_status_time"] = malware_status["@timestamp"]
            if malware_status["scan_status"] == SECRET_SCAN_STATUS_COMPLETED:
                total_scanned += 1
            elif malware_status["scan_status"] in "IN_PROGRESS":
                scan_in_progress += 1
        else:
            reg_image["malware_scan_status"] = cve_never_scanned
            reg_image["malware_scan_status_time"] = ""
            reg_image["malware_scan_status_msg"] = ""
...
def filter_node_for_malware_scan(node_filters):
    host_names = []
    node_names = []
    k8_names = []
    k8s_namespaces = node_filters.get("kubernetes_namespace", [])
